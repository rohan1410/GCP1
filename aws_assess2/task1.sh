aws s3 mb s3://permmsourcebucket --region us-east-1
aws s3 mb s3://permmdestinationbucket --region us-west-1
echo "Buckets Created.."

aws s3api put-bucket-versioning --bucket permmsourcebucket --versioning-configuration Status=Enabled
aws s3api put-bucket-versioning --bucket permmdestinationbucket --versioning-configuration Status=Enabled
...
def lambda_handler(event, context):
    ...
    target_bucket = 'permmdestinationbucket'
...
--function-name pe-rmm-copylambda \
--region "us-east-1" \
--statement-id "1" \
--action "lambda:InvokeFunction" \
--principal s3.amazonaws.com \
--source-arn arn:aws:s3:::permmsourcebucket 

echo "Getting ARN for the lambda function"
aws lambda get-function-configuration --function-name pe-rmm-copylambda \
